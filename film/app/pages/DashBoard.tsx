...
import { BasicCards } from "~/components/Card";
import EditModal from "~/components/Model";
import { fetchEntities, saveEntity, deleteEntity } from "~/api/api";
import { Pressable, View } from "react-native";
...
export default function Details() {
  ...
  return (
    <Container style={{ backgroundColor: '#350A24' }}>
      ...
      {showEditModal &&
        <EditModal data={data} closeEditModal={closeEditModal} isCreate={isCreateModal} header={header} confirmFormUpload={() => confirmFormUpload(modelo)} />
      }
    </Container>
  );
}